...
public class BanditEvaluator {
  ...
  public static BanditEvaluationResult evaluateBandit(
    ...
    Map<String, Double> actionWeights = weighActions(actionScores, modelData.getGamma(), modelData.getActionProbabilityFloor());
    String selectedActionKey = selectAction(flagKey, subjectKey, actionWeights);

    // Compute optimality gap in terms of score
    double topScore = actionScores.values().stream().mapToDouble(Double::doubleValue).max().orElse(0);
  ...
  private static Map<String, Double> scoreActions(DiscriminableAttributes subjectAttributes, Actions actions, BanditModelData modelData) {
  ...
  private static Map<String, Double> weighActions(Map<String, Double> actionScores, double gamma, double actionProbabilityFloor) {
    Double highestScore = null;
    String highestScoredAction = null;
    for (Map.Entry<String, Double> actionScore : actionScores.entrySet()) {
      if (highestScore == null || actionScore.getValue() > highestScore || actionScore.getValue().equals(highestScore) && actionScore.getKey().compareTo(highestScoredAction) < 0) {
  ...
  private static String selectAction(String flagKey, String subjectKey, Map<String, Double> actionWeights) {
    ...
    List<String> shuffledActionKeys = actionWeights.keySet().stream()
      .sorted(
        Comparator.comparingInt(
            (String actionKey) -> ShardUtils.getShard(flagKey + "-" + subjectKey + "-" + actionKey, BANDIT_ASSIGNMENT_SHARDS)
          )
          .thenComparing(actionKey -> actionKey))
      .collect(Collectors.toList());

    // Select action from the shuffled actions, based on weight
    double assignedShard = ShardUtils.getShard(flagKey + "-" + subjectKey, BANDIT_ASSIGNMENT_SHARDS);
