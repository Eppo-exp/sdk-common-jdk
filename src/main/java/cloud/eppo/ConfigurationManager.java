...
public class ConfigurationManager {
  ...
  private long cacheLoadTimeoutSeconds;

  ConfigurationManager(
      ConfigurationRequestor requestor,
      IConfigurationStore persistentSource,
      Configuration initialConfiguration) {
    this(requestor, persistentSource, initialConfiguration, 30);
  }
  ...
  ConfigurationManager(
      ...
      IConfigurationStore persistentSource,
      Configuration initialConfiguration,
      long cacheLoadTimeout) {
    this.requestor = requestor;
    this.configurationStore = persistentSource;
    this.cacheLoadTimeoutSeconds = cacheLoadTimeout;
  ...
  void load() {
    try {
      Configuration newConfig = requestor.loadFromRemote(config);
      setConfig(newConfig);
    } catch (RuntimeException e) {
      log.error("Failed to load configuration from remote", e);
      ...
      try {
        ...
        Configuration cachedConfig = bq.poll(cacheLoadTimeoutSeconds, TimeUnit.SECONDS);
        if (cachedConfig != null) {
          config = cachedConfig;
        } else {
          // Cache load failed to return before the timeout.
          log.debug("Cache load failed to return within the timeout period");
          throw new RuntimeException("Unable to load configuration from cache or remote");
        }
      } catch (InterruptedException ex) {
        throw new RuntimeException("Unable to load configuration from cache or remote", ex);
      }
    }
  }
          ...
          public void onSuccess(Configuration result) {
            synchronized (this) {
              try {
                ...
                // If fetching failed, and cache loading failed, fire off the failure callback if
                // not
                // yet done so
